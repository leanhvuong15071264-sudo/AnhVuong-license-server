    ...
    if (data.length) {
      list.innerHTML = data.map((item) => `
        <article class="download-card store-product-card">
          ...
          <div class="download-body">
            <div class="download-title-row">
              <h3>${esc(item.title)}${item.tag ? ` <span class="store-card-v2-tag" style="margin-left:6px;">${esc(item.tag)}</span>` : ''}</h3>
            ...
            ${(item.stock !== null && item.stock !== undefined) ? `<div class="download-description" style="margin-top:-4px;">Kho: <strong>${Number(item.stock)}</strong></div>` : ''}
...
function openStoreForm(item = null) {
  ...
  if ($('#storeTag')) { $('#storeTag').value = item?.tag || ''; }
